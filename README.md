...
1. **文章系统**
   - 📝 Markdown 文件通过 Content Collections 编译为 HTML
   - 📅 按日期排序
   - ⏱️ 阅读时间计算
   - 🔗 上下篇导航
   - 📰 RSS Feed：`/rss.xml`
...
3. **标签系统**
   ...
   - 📊 标签文章统计
   - 🔗 标签详情页
   - 🔁 标签会按规范化 slug 合并，例如 `Next.js` 与 `nextjs` 归入同一标签页
...
# 一键运行内容校验、ESLint、TypeScript、生产构建和 Algolia dry-run
npm run verify
```

内容由 Content Collections 生成。`npm run build` 会通过 Next adapter 自动生成内容；`npm run validate:content` 和 `npm run sync:algolia` 都会先运行 `npm run generate:content`，因此 clean checkout 也能校验内容并同步搜索索引。
...
在 `content/posts/` 目录下创建 `.md` 文件。当前项目不使用 Next MDX runtime；文章内容会由 Content Collections + `remark`/`rehype` 编译为 HTML。
...
- `date` 必须是合法日期，且不能晚于构建当天。
- `coverImage` 可留空；若填写，必须使用 `/images/` 下的本地资源路径，例如 `/images/cover.jpg`。图片文件需放在 `public/images/`，避免构建出缺失封面。
- 标签展示会保留文章中的原始写法，但标签页 URL 会按规范化 slug 合并。比如 `Next.js` 与 `nextjs` 会共同指向 `/tags/nextjs`。
